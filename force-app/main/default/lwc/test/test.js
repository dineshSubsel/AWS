...
export default class OrgBackupManager extends LightningElement {
    ...
    jobColumns = [
        ...
        { label: 'Start Time', fieldName: 'startTime', type: 'text' },
        {
            type: 'button',
            typeAttributes: {
                label: 'Cancel', name: 'cancel', variant: 'destructive',
    ...
    _loadAllStateUpfront() {
        const loaderKeys = [
            ...
            'lightningPages', 'fields', 'formulaFields'  // ← 'objects' removed
        ];
    ...
    loadObjects() {
        getAllObjects()
            .then(r => { this.rawObjectOptions = r.map(o => ({ label: o, value: o })); })
            .catch(e => console.error(e));
    }
    ...
    _loaderFor(loaderKey) {
        ...
        const map = {
            ...
            formulaFields: () => this._fetchMetadataByType('FormulaField'),  // ← ADD THIS LINE
            objects: () => Promise.resolve(this.rawObjectOptions || [])
        };

        if (!map[loaderKey]) {
            console.warn('[_loaderFor] No loader registered for key:', loaderKey);
            return Promise.resolve([]);
        }

        return map[loaderKey]().then(result => {
            this._loaderCache[cacheKey] = result;   // ← cache result for next call
            return result;
        });
    }
    ...
    // ─── Helper: read the latest copy of state for the current active page ────
    ...
    _loadSubItem(stateArr, gKey, sKey, setFn, page) {
        ...
        this._loaderFor(sub.loaderKey)
            ...
            .catch(e => {
                console.error('Load failed', gKey, sKey, e);
                const afterLoad = this._latestStateFor(resolvedPage, stateArr);
                const latestSub = (afterLoad.find(g => g.key === gKey)?.subs || [])
                    .find(s => s.key === sKey);
                setFn(afterLoad.map(g => g.key !== gKey ? g : {
    ...
    _loadChildSubItem(stateArr, gKey, parentSKey, csKey, setFn, page) {
        ...
        this._loaderFor(cs.loaderKey)
            ...
            .catch(e => {
                console.error('Child load failed', gKey, parentSKey, csKey, e);
                const afterLoad = this._latestStateFor(resolvedPage, stateArr);
                const latestCs = afterLoad.find(g => g.key === gKey)
                    ?.subs.find(s => s.key === parentSKey)
                    ?.childSubs?.find(c => c.key === csKey);
        ...
        const updated = stateArr.map(g => {
            ...
            return {
                ...
                subs: g.subs.map(s => ({
                    ...
                    expanded: true,
                    selected: s.loaded && s.pool.length > 0
                        ? s.pool.map(i => i.value)
                        : s.selected,
                    pendingSelect: !s.loaded,
                    childSubs: s.childSubs?.map(cs => ({
                        ...cs,
                        checkboxClicked: true,
        ...
        setTimeout(() => {
            group.subs.forEach(s => {
                if (!s.loaded && !s.loading) {
                    this._loadSubItem(stateArr, gKey, s.key, setFn, page);
                }
                s.childSubs?.forEach(cs => {
                    if (!cs.loaded && !cs.loading) {
                        this._loadChildSubItem(stateArr, gKey, s.key, cs.key, setFn, page);
                    }
                });
            });
        }, 0);
    ...
    _buildNavTree(stateArr, activeGKey, activeSKey, activeCSKey) {
        ...
        return stateArr.map(g => {
            ...
            const totalPool = g.subs.reduce((sum, s) => {
                let t = s.pool ? s.pool.length : 0;
                if (s.childSubs) t += s.childSubs.reduce((acc, c) => acc + (c.pool ? c.pool.length : 0), 0);
                return sum + t;
            }, 0);

            const allChecked = totalPool > 0 && totalSelected === totalPool;
            const someChecked = totalSelected > 0;
    ...
    _subBadgeClass(sub) {
        ...
        if (sub.loaded && sub.pool.length === 0) return 'nav-sub-badge nav-sub-badge--zero';
    ...
    // ── Backup/restore disabled getters ───────────────────────────────────────
    ...
    handleScheduleTimeChange(event) { this.scheduleTime = event.target.value; }
    handleWeekdayChange(event) { this.scheduleWeekday = event.target.value; }
    handleMonthDayChange(event) { this.scheduleMonthDay = parseInt(event.target.value, 10) || 1; }

    handleRestoreScheduleSelect(event) { this.restoreScheduleMode = event.currentTarget.dataset.freq; }
    handleRestoreScheduleTimeChange(event) { this.restoreScheduleTime = event.target.value; }
    handleRestoreWeekdayChange(event) { this.restoreScheduleWeekday = event.target.value; }
    handleRestoreMonthDayChange(event) { this.restoreScheduleMonthDay = parseInt(event.target.value, 10) || 1; }
    ...
    _applyUpfrontPools(stateArr) {
        return stateArr.map(g => ({
            ...g,
            subs: g.subs.map(s => {
                if (s.loaderKey === 'objects') return { ...s, loaded: false, loading: false }; // ← never pre-load
    ...
    // ── Sub item click / checkbox handlers ────────────────────────────────────
    ...
    handleOrgFullSubItemClick(event) {
        ...
        if (sKey === 'dataObjects') {
            // Step 1: Set active keys so right panel opens
            ...
            // Step 3: Load pool directly from rawObjectOptions (bypass _loadSubItem entirely)
            setTimeout(() => {
    ...
    // ── Data object/record multiselect getters ────────────────────────────────
    ...
    get isDataBackupDisabled() { return !this.hasDataSelectedObjects; }
    ...
    get isRestoreDataDisabled() { return !this.hasRestoreDataSelectedObjects; }
    ...
    // ── Data object/record dropdown handlers ──────────────────────────────────
    ...
    _onBackupStarted(jobId, label, mode, scheduleMode) {
        const modeLabel = mode === 'instant' ? 'instant' : `scheduled (${scheduleMode})`;
        this._toast('success', `${label} Started`, `Backup initiated. Mode: ${modeLabel}`);
    ...
    startOrgFullBackup() {
        ...
        const objSel = dataGroup?.subs[0]?.selected ?? [];
    ...
    startRestore() {
        ...
        const onSuccess = result => {
            ...
            if (!msg) return;
            this._toast('success', 'Restore Started', `${msg} Mode: ${this.selectedRestoreMode}`);
            this._resetRestorePage();
            this._resetRestoreSchedule();
            this.handleRestoreBack();
            if (this.selectedRestoreMode === 'instant' && jobId) {
                this._activeJobIds = [jobId];
        ...
        const onError = e => this._toast('error', 'Error', e?.body?.message || 'An error occurred');

        if (type === 'data') {
            if (!this.hasRestoreDataSelectedObjects) {
            ...
            startGroupRestoreApex({
                ...
                groupPayload: JSON.stringify({
                    objects: this.restoreDataSelectedObjects,
                    recordIds: this.restoreDataSelectedRecords,
                    fileIds: this.restoreSelectedFileIds
                }),
                schedulePayload
            ...
            startGroupRestoreApex({ restoreType: type, groupPayload: JSON.stringify(payload), schedulePayload })
                .then(onSuccess).catch(onError);
        }
    }

    _parseRestoreResult(raw) {
        if (!raw) return { msg: null, jobId: null };
        try {
            const parsed = JSON.parse(raw);
            if (parsed?.msg) {
                if (parsed.msg.startsWith('ERROR:')) {
                    this._toast('error', 'Error', parsed.msg.replace('ERROR:', '').trim());
        ...
        if (raw.startsWith('ERROR:')) {
            this._toast('error', 'Error', raw.replace('ERROR:', '').trim());
            return { msg: null, jobId: null };
        }
    ...
    // ── Polling ───────────────────────────────────────────────────────────────
    // ── FIX: single _checkJobStatus tick (matches FileMigration exactly) ──────

    _startPolling() {
        this._stopPolling();
        this._fetchJobRows();   // immediate first fetch so table shows at once
        ...
        getBackupJobStatus({ jobIds: this._activeJobIds })
            .then(rows => {
                if (!rows || rows.length === 0) {
                    // Job may already be gone from the active query — keep last known rows
                    return;
                }
            ...
            .catch(err => console.error('Job row fetch error', err));
    }

    // Single tick: fetch rows THEN check done — no racing intervals
    _checkJobStatus() {
        this._fetchJobRows();
        ...
        areAllJobsDone({ jobIds: this._activeJobIds })
            .then(done => {
                if (done) {
                    ...
                    this._activeJobIds = [];
                    this._toast('success', 'Backup Complete', 'All backup jobs finished successfully.');
                }
            })
    ...
    get computedDataRecordClass() {
        ...
        // No records found — extra grey styling
    ...
    handleRecordClick(event) {
        // No records available OR no object selected — block completely
